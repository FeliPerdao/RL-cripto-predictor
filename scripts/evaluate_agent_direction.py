...
def evaluate_agent_direction(model_path, data, predict_steps=3, tf_name=""):
    ...
    for i in range(predict_steps):
        ...
        plt.plot(np.sign(predictions[-50:]), label="Dirección Predicha", color="blue")
        plt.plot(np.sign(reals[-50:]), label="Dirección Real", color="green")
        plt.plot(np.cumsum(rewards[-50:]), label="Reward acumulado", color="red", linestyle="--")
